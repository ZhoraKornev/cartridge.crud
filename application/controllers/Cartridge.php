...
class Cartridge extends CI_controller
{
    ...
    public function updatedetails($id)
    {
        ...
        $this->data = array(
            ...
            'inservice' => ($this->input->post('date_income') < $this->input->post('date_outcome') ? 1 :0));
        if ($_POST)
        {
            //$this->update_log($id,$this->data);
    ...
    public function story_update($id,$data_from_form){
        ...
        foreach ($data_from_form as $key=>$value) {
            if (isset($item_from_db[$key]))
            {
                ...
                {
                    $logtmp = " Новое событие ".date("$d.$m.$y")." ключ ||".$key."|| с данными  **".$item_from_db[$key]."**изменилось на**".$value."** \n";
                    $log .= $logtmp;
                }
            }
        ...
        $log_full =  $log_full_old."  "."Изменения были ".date("$d.$m.$y")." данные: **".$current_info_db."** изменились на новые данные  **".$new_info."** конец. \n";
